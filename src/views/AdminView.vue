      ...
      <div class="sidebar p-4 d-flex flex-column justify-content-between flex-shrink-0" style="width: 280px;">
        <div>
          ...
          <div class="list-group list-group-flush bg-transparent">
            ...
            <button @click="currentTab = 'hechos'" :class="['list-group-item list-group-item-action bg-transparent text-start', { 'active': currentTab === 'hechos' }]">Historial de Hechos</button>
          </div>
        </div>
        <div>
      ...
      <div class="container-fluid p-5 d-flex flex-column overflow-auto">
              ...
              <table class="table table-borderless align-middle mb-0">
                ...
                <tbody>
                  <tr v-for="t in myAssignedTickets" :key="t.id" class="border-bottom">
                    ...
                    <td>{{ t.subject }}</td>
                    <td><button @click="marcarComoHecho(t.id)" class="btn btn-sm btn-success px-3 rounded-pill">✔ Hecho</button></td>
                  </tr>
                  <tr v-if="myAssignedTickets.length === 0">
                    <td colspan="5" class="text-center py-5 text-muted">No tienes tickets asignados en cola.</td>
                  </tr>
                </tbody>
              </table>
            </div>
        ...
        <div v-if="currentTab === 'disponibles'">
          ...
          <div class="card p-3">
            <table class="table table-borderless align-middle mb-0">
              <thead class="border-bottom">
                <tr><th class="ps-3 text-muted">ID</th><th class="text-muted">Prioridad</th><th class="text-muted">Autor</th><th class="text-muted">Asunto</th><th class="text-muted">Fecha</th><th class="text-muted">Acción</th></tr>
              </thead>
              <tbody>
                <tr v-for="t in filteredUnassigned" :key="t.id" class="border-bottom">
                  <td class="ps-3 fw-bold">{{ t.id }}</td>
                  <td><span class="badge bg-secondary px-3 py-2 rounded-pill text-uppercase">{{ t.priority }}</span></td>
                  <td class="text-capitalize">{{ t.author }}</td>
                  <td>{{ t.subject }}</td>
                  <td class="small text-muted">{{ t.createdAt.toLocaleTimeString() }}</td>
                  <td><button @click="tomarTicket(t.id)" class="btn btn-sm btn-outline-primary px-3 rounded-pill">Tomar</button></td>
                </tr>
                <tr v-if="filteredUnassigned.length === 0">
                   <td colspan="6" class="text-center py-5 text-muted">No hay tickets pendientes actualmente.</td>
                </tr>
              </tbody>
            </table>
          </div>
        ...
        <div v-if="currentTab === 'hechos'">
          <h2 class="h3 mb-4 fw-bold">Archivo de Tickets Hechos</h2>
          <div class="card bg-white p-3">
            <div class="card-body p-0">
              <table class="table table-borderless align-middle mb-0">
                <thead class="border-bottom">
                  <tr><th class="ps-3 text-muted">ID</th><th class="text-muted">Autor</th><th class="text-muted">Asunto</th><th class="text-muted">Estado</th></tr>
                </thead>
                <tbody>
                  <tr v-for="t in store.closedTickets" :key="t.id" class="border-bottom">
                    <td class="ps-3 fw-bold text-muted">{{ t.id }}</td>
                    <td class="text-capitalize">{{ t.author }}</td>
                    <td>{{ t.subject }}</td>
                    <td><span class="badge bg-success bg-opacity-10 text-success px-3 py-2 rounded-pill">HECHO</span></td>
                  </tr>
                  <tr v-if="store.closedTickets.length === 0">
                    <td colspan="4" class="text-center py-5 text-muted">Aún no hay tickets marcados como hechos.</td>
                  </tr>
                </tbody>
              </table>
            </div>
...
const myAssignedTickets = computed(() => store.tickets.filter(t => t.assignedTo === store.currentUser?.fullName && t.status !== 'hecho'))
...
const userTickets = (authorName) => {
  return [...store.tickets, ...store.closedTickets].filter(t => t.author === authorName)
}

// Nueva llamada a la función del store
const tomarTicket = (id) => {
  store.acceptTicket(id)
}

const marcarComoHecho = (id) => {
  store.closeTicket(id)
}
